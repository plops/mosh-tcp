...
#[test]
fn test_c_client_integration() -> anyhow::Result<()> {
    // 1. Compile C client
    let make_status = Command::new("make")
        .args(&["-C", "clients/c"])
        .status()?;
    assert!(make_status.success(), "Failed to build mosh-tcp-client-c");
    ...
    let mut client_proc = Command::new("./clients/c/mosh-tcp-client-c")
    ...
    let test_cmd = b"echo C_CLIENT_INTEGRATION_TEST_PASSED\n";
    ...
    while Instant::now() < deadline {
        ...
        if text.contains("C_CLIENT_INTEGRATION_TEST_PASSED") {
            found = true;
            break;
        }
